...
if (!convexUrl || !clientKey)
  throw new Error('Source .env.local before running the seed script.');
const client = new ConvexHttpClient(convexUrl);
const sourceVideos = [
  'https://download.blender.org/durian/trailer/sintel_trailer-480p.mp4',
  'https://download.blender.org/peach/trailer/trailer_iphone.m4v',
];
const work = join(tmpdir(), 'move-sync-real-demo');
...
async function uploadBlob(mediaId, blob, contentType) {
  const url = await client.mutation(
    api.media.generateUploadUrl,
    mediaId ? { clientKey, id: mediaId } : { clientKey },
  );
  const response = await fetch(url, {
    method: 'POST',
...
for (const sourceUrl of sourceVideos) {
  ...
  const localAssetId = `real-demo:${filename}`;
  if (
    await client.query(api.media.hasLocalAsset, { clientKey, localAssetId })
  ) {
    console.log(`Already synced: ${filename}`);
    continue;
  }
  const path = join(work, filename);
  const thumb = join(work, `${filename}.jpg`);
  await download(sourceUrl, path);
  ...
  if (frame.status !== 0)
    throw new Error('Unable to extract an actual video frame.');
  const common = {
    clientKey,
    localAssetId,
    filename,
  ...
  const mediaId = await client.mutation(api.media.enqueue, common);
  ...
  const thumbnailStorageId = await uploadBlob(
    undefined,
    new Blob([readFileSync(thumb)]),
    'image/jpeg',
  );
  await client.mutation(api.media.completeUpload, {
    ...common,
    id: mediaId,
    storageId,
    thumbnailStorageId,
  });
  console.log(`Synced through real storage flow: ${filename}`);
}
