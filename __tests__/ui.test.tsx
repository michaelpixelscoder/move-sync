...
describe('shared UI and formatters', () => {
  ...
  it('opens a media card and supports long-press selection', () => { const onPress = jest.fn(); const onLongPress = jest.fn(); const ui = render(<MediaCard item={item} desktop={false} selected={false} onPress={onPress} onLongPress={onLongPress} />); fireEvent.press(ui.getByLabelText('VID_20260831.mp4, synced')); fireEvent(ui.getByLabelText('VID_20260831.mp4, synced'), 'longPress'); expect(onPress).toHaveBeenCalledTimes(1); expect(onLongPress).toHaveBeenCalledTimes(1); });
