...
const item = {
  _id: 'media-id',
  filename: 'VID_20260831.mp4',
  mimeType: 'video/mp4',
  sizeBytes: 1_572_864,
  durationMs: 65_000,
  createdAt: 1_788_131_200_000,
  locationName: null,
  width: 1920,
  height: 1080,
  collectionId: null,
  collectionName: 'Camera',
  sourceCollectionLocalId: 'device:camera',
  deviceId: null,
...
describe('shared UI and formatters', () => {
  it('formats actual media metadata for the UI', () => {
    expect(formatBytes(1_572_864)).toBe('1.5 MB');
    expect(formatDuration(65_000)).toBe('1:05');
    expect(titleFromFilename('VID_20260831.mp4')).toBe('VID 20260831');
  });
  it('uses human capture dates that stay useful as media ages', () => {
    const now = new Date(2026, 8, 2, 10, 30);
    expect(
      formatMediaCaptureDate(new Date(2026, 8, 2, 8, 30).getTime(), now),
    ).toBe('2 hours ago');
    expect(
      formatMediaCaptureDate(new Date(2026, 8, 2, 1, 0).getTime(), now),
    ).toBe('Today, 1:00 am');
    expect(
      formatMediaCaptureDate(new Date(2026, 7, 23, 12, 0).getTime(), now),
    ).toBe('23 August');
  });
  it('uses customer-facing backup state language', () => {
    expect(backupStateLabel('synced')).toBe('Backed up');
    expect(backupStateLabel('queued')).toBe('Waiting to upload');
    expect(backupStateLabel('error', 'Network unavailable')).toBe(
      'Network unavailable',
    );
  });
  it('opens a media card and supports long-press selection', () => {
    ...
    expect(ui.getByText(/· 1\.5 MB/)).toBeTruthy();
    fireEvent.press(ui.getByLabelText('VID_20260831.mp4, Backed up'));
    fireEvent(ui.getByLabelText('VID_20260831.mp4, Backed up'), 'longPress');
  ...
  it('keeps missing thumbnails, capture metadata, and upload progress intentional', () => {
    const uploading = {
      ...item,
      filename:
        'A very long movement rehearsal title that should remain readable.mp4',
  ...
  it('prevents disabled actions from firing', () => {
    const onPress = jest.fn();
    const ui = render(<Button label="Preparing…" disabled onPress={onPress} />);
  ...
  it('uses the S4 desktop side rail and the S3 mobile navigation contract', () => {
    ...
    const desktop = render(
      <AppNavigation
        clientKey="owner-8e60fc22-3f3d-4e42-a647-fcb72cb58811"
        desktop
        screen={{ name: 'videos' }}
