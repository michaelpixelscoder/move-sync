...
export const storageStateValidator = v.union(
  v.literal('uploading'),
  v.literal('backedUp'),
  v.literal('onDeviceAndCloud'),
  v.literal('cloudOnly'),
  v.literal('waiting'),
  v.literal('failed'),
);
export const activityStateValidator = v.union(
  v.literal('waiting'),
  v.literal('uploading'),
  v.literal('completed'),
  v.literal('failed'),
...
export const storageValidator = v.object({
  state: storageStateValidator,
  cloudAvailable: v.boolean(),
  localAvailable: v.boolean(),
  safeToRemoveLocal: v.boolean(),
  backedUpAt: v.union(v.number(), v.null()),
});
export const mediaValidator = v.object({
  ...
  sourceCollectionLocalId: v.union(v.string(), v.null()),
  ...
  transferState: syncStateValidator,
...
export const librarySummaryValidator = v.object({
...
export const backupActivityValidator = v.object({
  _id: v.id('backupActivities'),
  _creationTime: v.number(),
  mediaId: v.union(v.id('media'), v.null()),
  filename: v.string(),
  state: activityStateValidator,
  progress: v.number(),
  attempt: v.number(),
