...
describe('Move Sync backend', () => {
  ...
  it('derives independent local and cloud storage facts and never permits premature local removal', async () => {
    const metadata = { clientKey: ownerKey, localAssetId: 'safety-video', filename: 'safety.mp4', mimeType: 'video/mp4', sizeBytes: 120, durationMs: 1000, createdAt: 200 };
    const id = await t.mutation(api.media.enqueue, metadata);
    expect((await t.query(api.media.getById, { clientKey: ownerKey, id })).storage).toMatchObject({ state: 'waiting', cloudAvailable: false, localAvailable: true, safeToRemoveLocal: false });
    await expect(t.mutation(api.media.markLocalRemoved, { clientKey: ownerKey, id })).rejects.toThrow(/verified cloud copy/i);
    await t.mutation(api.media.generateUploadUrl, { clientKey: ownerKey, id });
    expect((await t.query(api.media.getById, { clientKey: ownerKey, id })).storage.state).toBe('uploading');
    const storageId = await storedVideo();
    await t.mutation(api.media.completeUpload, { ...metadata, id, storageId });
    const backedUp = await t.query(api.media.getById, { clientKey: ownerKey, id });
    expect(backedUp.storage).toMatchObject({ state: 'onDeviceAndCloud', cloudAvailable: true, localAvailable: true, safeToRemoveLocal: true });
    await t.mutation(api.media.markLocalRemoved, { clientKey: ownerKey, id });
    expect((await t.query(api.media.getById, { clientKey: ownerKey, id })).storage).toMatchObject({ state: 'cloudOnly', cloudAvailable: true, localAvailable: false, safeToRemoveLocal: false });
  });

  it('persists retryable activity and summary aggregates without client-side reductions', async () => {
    const metadata = { clientKey: ownerKey, localAssetId: 'activity-video', filename: 'activity.mp4', mimeType: 'video/mp4', sizeBytes: 999, durationMs: 1000, createdAt: 300 };
    const id = await t.mutation(api.media.enqueue, metadata);
    await t.mutation(api.media.generateUploadUrl, { clientKey: ownerKey, id });
    await t.mutation(api.media.updateUploadProgress, { clientKey: ownerKey, id, progress: .4 });
    await t.mutation(api.media.markSyncError, { clientKey: ownerKey, id, message: 'Network unavailable' });
    let activity = await t.query(api.activity.listPage, { clientKey: ownerKey, paginationOpts: { cursor: null, numItems: 10 } });
    expect(activity.page[0]).toMatchObject({ mediaId: id, state: 'failed', error: 'Network unavailable', attempt: 1 });
    await t.mutation(api.media.retryUpload, { clientKey: ownerKey, id });
    activity = await t.query(api.activity.listPage, { clientKey: ownerKey, paginationOpts: { cursor: null, numItems: 10 } });
    ...
    expect(summary).toMatchObject({ cloudVideoCount: 1, backedUpCount: 1, reclaimableCount: 1, failedCount: 0, activeUploadCount: 0, waitingCount: 0 });
  });

  it('paginates indexed library discovery and preserves the stable collection relation', async () => {
    const [camera] = await t.mutation(api.collections.reconcile, { clientKey: ownerKey, collections: [{ localId: 'ios:camera', name: 'Camera', assetCount: 2, videoCount: 2 }] });
    ...
    await t.mutation(api.collections.reconcile, { clientKey: ownerKey, collections: [] });
    expect((await t.query(api.collections.list, { clientKey: ownerKey }))[0].isAvailable).toBe(false);
  });
