...
import { AutoSyncScreen } from './features/autosync/screens/AutoSyncScreen';
...
function MoveSync() {
  const desktop = useWindowDimensions().width >= 800; const [screen, setScreen] = useState<Screen>({ name: 'media' }); const { clientKey, error } = useClientKey();
  ...
  const content = <View style={styles.main}>{screen.name === 'media' ? <MediaLibraryScreen clientKey={clientKey} onOpen={mediaId => setScreen({ name: 'player', mediaId })} /> : screen.name === 'autosync' ? <AutoSyncScreen clientKey={clientKey} /> : <PlayerScreen clientKey={clientKey} mediaId={screen.mediaId} onBack={() => setScreen({ name: 'media' })} />}</View>;
