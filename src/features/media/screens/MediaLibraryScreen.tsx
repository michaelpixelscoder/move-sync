...
export function MediaLibraryScreen({ clientKey, onOpen }: Props) {
  ...
  const [filter, setFilter] = useState<'synced' | 'queue'>('synced'); const [selected, setSelected] = useState<Set<Id<'media'>>>(new Set()); const [uploads, setUploads] = useState<UploadProgress[]>([]); const [error, setError] = useState<string>(); const [sharing, setSharing] = useState(false);
  const shown = useMemo(() => (rows ?? []).filter(item => filter === 'synced' ? item.state === 'synced' : item.state !== 'synced'), [filter, rows]);
  ...
  if (rows === undefined) return <LoadingState label="Loading your cloud library…" />;
  return <View style={styles.screen}><View style={[styles.header, desktop && styles.desktopFrame]}><View><Text style={styles.eyebrow}>MOVE SYNC</Text><Text accessibilityRole="header" style={styles.heading}>Medias</Text></View><Button label={desktop ? 'Upload videos' : 'Upload'} icon="cloud-upload-outline" onPress={pickVideos} /></View>
    {error ? <ErrorState message={error} /> : null}
    <View style={[styles.toolbar, desktop && styles.desktopFrame]}><View accessibilityRole="tablist" style={styles.tabs}>{(['synced', 'queue'] as const).map(value => <Pressable key={value} accessibilityRole="tab" accessibilityState={{ selected: filter === value }} onPress={() => setFilter(value)} style={[styles.tab, filter === value && styles.tabActive]}><Text style={[styles.tabText, filter === value && styles.tabTextActive]}>{value === 'synced' ? 'Synced' : 'Queue'}</Text></Pressable>)}</View>{selected.size ? <IconButton label="Clear selection" name="close" onPress={() => setSelected(new Set())} /> : null}</View>
    ...
    {shown.length ? <ScrollView contentContainerStyle={[styles.grid, desktop && styles.desktopFrame, !desktop && styles.list]}>{shown.map(item => <MediaCard key={item._id} item={item as MediaRecord} desktop={desktop} selected={selected.has(item._id)} onLongPress={() => toggle(item._id)} onPress={() => selected.size ? toggle(item._id) : onOpen(item._id)} />)}</ScrollView> : <EmptyState title={filter === 'synced' ? 'No synced videos yet' : 'The queue is clear'} message={filter === 'synced' ? 'Choose real video files from this device. They will be uploaded to Convex Storage and appear here when complete.' : 'New uploads and automatic collection syncs appear here while they are running.'} action={filter === 'synced' ? 'Choose videos' : undefined} onAction={filter === 'synced' ? pickVideos : undefined} />}
    {selected.size ? <View style={styles.selection}><View><Text style={styles.selectionTitle}>{selected.size} selected</Text><Text style={styles.selectionMeta}>Cloud videos ready to send</Text></View><Button label={sharing ? 'Preparing…' : 'Share'} icon="share-outline" disabled={sharing} onPress={share} /></View> : null}
  </View>;
}
