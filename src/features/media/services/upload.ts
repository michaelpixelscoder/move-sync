...
type CompleteMetadata = { collectionId?: Id<'collections'>; sourceCollectionLocalId?: string; localAssetId?: string; filename: string; mimeType: string; sizeBytes: number; durationMs: number; createdAt: number; locationName?: string; width?: number; height?: number };
...
async function sendToStorage(clientKey: string, source: UploadSource, mediaId?: Id<'media'>): Promise<Id<'_storage'>> {
  ...
  if (Platform.OS === 'web') {
    ...
    source.onProgress?.(1); if (mediaId) await convex.mutation(api.media.updateUploadProgress, { clientKey, id: mediaId, progress: 1 });
    return (await response.json()).storageId as Id<'_storage'>;
  }
  const file = new ExpoFile(source.uri);
  let persistedProgress = 0;
  const task = file.createUploadTask(uploadUrl, { httpMethod: 'POST', uploadType: UploadType.BINARY_CONTENT, mimeType: source.mimeType, headers: { 'Content-Type': source.mimeType }, onProgress: ({ totalBytes, bytesSent }) => { const progress = totalBytes > 0 ? bytesSent / totalBytes : 0; source.onProgress?.(progress); if (mediaId && progress - persistedProgress >= .05) { persistedProgress = progress; void convex.mutation(api.media.updateUploadProgress, { clientKey, id: mediaId, progress }).catch(() => undefined); } } });
...
async function complete(clientKey: string, source: UploadSource, metadata: CompleteMetadata) {
  ...
  try { const storageId = await sendToStorage(clientKey, source, mediaId); const thumbnail = await makeThumbnail(source.uri, source.webBody instanceof globalThis.File ? source.webBody : undefined).catch(() => null); const thumbnailStorageId = thumbnail?.blob ? await sendToStorage(clientKey, { uri: '', mimeType: 'image/jpeg', webBody: thumbnail.blob }) : thumbnail?.uri ? await sendToStorage(clientKey, { uri: thumbnail.uri, mimeType: 'image/jpeg' }) : undefined; return await convex.mutation(api.media.completeUpload, { clientKey, id: mediaId, ...metadata, storageId, thumbnailStorageId }); }
  catch (error) { const message = error instanceof Error ? error.message : 'Upload failed'; await convex.mutation(api.media.markSyncError, { clientKey, id: mediaId, message }).catch(() => undefined); throw error; }
}
