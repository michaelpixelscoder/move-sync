...
export async function readPickedVideoMetadata(
  ...
  if (!file)
    return { durationMs: 0, createdAt: asset.lastModified || Date.now() };
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      video.onerror = () => reject(new Error(`Unable to read ${asset.name}`));
    });
    return {
      ...
      createdAt: asset.lastModified || Date.now(),
    };
  } finally {
