...
export async function syncCollection(clientKey: string, collection: { _id: import('../../../../convex/_generated/dataModel').Id<'collections'>; localId: string; name: string }, onItem?: (filename: string) => void) {
  ...
  for (const asset of assets) {
    ...
    onItem?.(info.filename); await uploadDeviceAsset(clientKey, info, collection, locationName); uploaded += 1;
  }
  return uploaded;
}
