...
- [x] The backend derives ownership from a verified authentication identity and rejects unauthenticated access to private data.
- [x] Every supplied document or storage identifier is checked against the authenticated owner before use.
- [ ] A user can sign in on every supported platform, use one library on multiple devices, sign out safely, and recover access. Multi-device and sign-out behavior are implemented; interactive platform and recovery-delivery acceptance remain.
- [x] Users and device installations remain separate records and device-specific metadata remains attributable.
- [x] The legacy library claim is single-owner, idempotent, replay-safe, and account aliases expose one canonical library. Production migration observation remains part of release staging.
- [x] Account deletion has a tested atomic policy for database records and stored video objects. Accounts above the bounded immediate-deletion limit fail without deleting data.
...
| Configure Google OAuth             | Development Google client points to the Convex HTTP callback and deployment credentials are set                    | Google Cloud project credentials      | active    | OAuth initiation is verified; interactive web and native callback checks remain                   |
| Add verified email recovery        | Password reset sends a short-lived code through Resend and rotates all other sessions after reset                  | `AUTH_RESEND_KEY` and approved sender | blocked   | UI and provider flow are implemented; delivery cannot be accepted until credentials are supplied  |
| Prototype authenticated ownership  | Sessions reach Convex functions whose identity is derived server-side                                              | Authentication shell                  | completed | Automated tests prove changing request arguments cannot impersonate another user                  |
| Complete threat model              | Identity mapping, explicit linking rules, and authorization invariants are enforced and reviewed                   | Session and ownership prototypes      | completed | Central authorization wrapper and Convex security review pass                                     |
| Introduce users and devices        | Internal users own libraries; installations remain distinct device records                                         | Selected solution                     | completed | Multi-device backend test proves shared library and distinct device IDs                           |
| Migrate backend authorization      | All private functions and storage flows resolve the authenticated user internally                                  | User model and threat model           | completed | Function inventory is complete; unauthenticated and cross-user suites pass                        |
| Implement client account lifecycle | Sign-in, restoration, sign-out/account rotation, recovery UI, and other-session revocation are implemented         | Backend authorization and provider UI | active    | Interactive platform and recovery-delivery acceptance remain                                      |
| Handle legacy libraries            | Libraries are claimed through a single-owner, replay-safe mapping to one canonical account library                 | Authenticated ownership               | completed | Idempotency, replay rejection, alias, and multi-device tests pass                                 |
| Implement deletion lifecycle       | Account deletion atomically removes bounded database rows, auth records, and stored video objects                  | Authenticated ownership               | completed | Success and cross-user preservation tests pass; oversized accounts fail before deletion           |
...
| 2026-09-17 | Enforce claims in one authorization wrapper                 | Every private domain function now requires a verified Convex Auth user and maps its installation claim to the canonical account library; caller keys alone grant no access.                       |
| 2026-09-17 | Preserve installation identity separately                   | Multiple claims for one account resolve to the same library while device records retain the installation key used to create them.                                                                 |
| 2026-09-17 | Rotate installation identity on explicit sign-out           | This prevents a new account on the same client from inheriting the previous account's claim or cached component state.                                                                            |
| 2026-09-17 | Use atomic immediate account deletion with a safety cap     | Database and storage deletion succeeds together for up to 500 related documents; larger accounts are rejected untouched until a batched deletion job is introduced.                               |
| 2026-09-17 | Keep recovery acceptance open pending email credentials     | The reset request, code verification, password replacement, and session invalidation flow is implemented, but no `AUTH_RESEND_KEY` or approved sender is configured.                              |
